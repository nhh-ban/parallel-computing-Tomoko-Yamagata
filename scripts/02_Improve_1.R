#Solution file 02_
#The solution script where you rewrite lines 29-35 
#(i.e. the final loop) to use parallel computing.
...
# Modified the codes of loop by using foreach()function
df$share_reject <- 
  foreach(
    i = 1:nrow(df), .combine = c) %dopar% {
      result <- MTweedieTests(
        N = df$N[i],
        M = df$M[i],
        sig = 0.05
        )
